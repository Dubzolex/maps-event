...
@Composable
fun ModifyEventSheet(
    event: Events,
    onClose: () -> Unit,
    onModify: (Events) -> Unit
) {
    var name by remember { mutableStateOf(event.name) }
    var description by remember { mutableStateOf(event.description) }
    var type by remember { mutableStateOf(event.type) }
    var date by remember { mutableStateOf(event.date) }
    var ville by remember { mutableStateOf(event.ville) }
    var adresse by remember { mutableStateOf(event.adresse) }
    ...
    LazyColumn(
        ...
        item {
            Text(text = "Modifier un événement")
        }
